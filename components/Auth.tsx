  ...
  const handleProviderSignIn = async (provider: any, providerName: string) => {
    ...
    try {
      ...
      if (err.code === 'auth/popup-blocked') {
        setError("Sign-in popup was blocked. Please enable popups or try again.");
        ...
        setError(`Failed to sign in with ${providerName}. ${err.message || 'Please try again.'}`);
      }
    } finally {
      setLoading(false);
    }
              ...
              {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : (
                <>
                  <svg className="w-5 h-5" viewBox="0 0 24 24">
                    ...
                    <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1c-4.3 0-7.9 2.53-9.57 6.27L5.84 10c.9-2.66 3.4-4.62 6.16-4.62z" fill="#EA4335"/>
                  </svg>
                  Google
                </>
              )}
