...
const YahooIcon = ({ className }: { className?: string }) => (
  <svg className={className} viewBox="0 0 512 512" fill="currentColor">
    <path d="M224 96l-64 160h-64L160 96h64zm224 0l-64 160h-64l64-160h64zM272 288h-32l-32 80H160l64-160h64l64 160h-48l-32-80zm-48 0h32l-16 40-16-40z" fill="none"/> {/* Placeholder shape adjustment */}
    <path d="M414.5 96L284.6 309.8 284.4 416h-59.5l-.2-105.8L97.5 96h62.6l91.7 166.7L348.6 96h65.9z"/>
  </svg>
);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (view === 'login') {
        ...
        const allowedDomains = ['@gmail.com', '@outlook.com', '@microsoft.com', '@yahoo.com'];
        const isValidDomain = allowedDomains.some(domain => email.toLowerCase().endsWith(domain));
        
        if (!isValidDomain) {
          setError("Sign up is restricted to trusted email providers (Gmail, Outlook, Yahoo).");
          setLoading(false);
          return;
        }
          ...
          <div className="flex flex-col gap-3 mb-6">
            <div className="grid grid-cols-2 gap-3">
              <button
                ...
                disabled={loading}
                className="flex items-center justify-center gap-2 py-2.5 px-4 bg-white border border-slate-200 rounded-xl font-semibold text-slate-700 hover:bg-slate-50 transition-all shadow-sm text-xs"
              >
              ...
              <button
                  ...
                  disabled={loading}
                  className="flex items-center justify-center gap-2 py-2.5 px-4 bg-black border border-black rounded-xl font-semibold text-white hover:bg-gray-900 transition-all shadow-sm text-xs"
              >
            ...
            <div className="grid grid-cols-2 gap-3">
              <button
                  ...
                  disabled={loading}
                  className="flex items-center justify-center gap-2 py-2.5 px-4 bg-[#6001D2] border border-[#6001D2] rounded-xl font-semibold text-white hover:bg-[#5000B0] transition-all shadow-sm text-xs"
              >
              ...
              <button
                  ...
                  disabled={loading}
                  className="flex items-center justify-center gap-2 py-2.5 px-4 bg-slate-900 border border-slate-800 rounded-xl font-semibold text-white hover:bg-slate-800 transition-all shadow-sm text-xs"
              >
