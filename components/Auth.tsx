...
const Auth: React.FC<AuthProps> = ({ onSignUpSuccess, onClose }) => {
  ...
  const handleMagicLinkSubmit = async (e: React.FormEvent) => {
    ...
    const actionCodeSettings = {
      // Must point back to your app's URL
    ...
    try {
      ...
      // Save the email locally so you don't have to ask the user for it again on the same device
      window.localStorage.setItem('emailForSignIn', email);
      setLinkSent(true);
    } catch (err: any) {
      console.error(err);
      setError(err.message || "Failed to send magic link. Please try again later.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="relative bg-white p-8 rounded-2xl shadow-2xl border border-slate-100 w-full max-w-md animate-in fade-in zoom-in duration-300">
        ...
        <div className="mb-8 text-center">
          ...
          <p className="text-slate-500 mt-2 text-sm">
            Enter your email and we'll send you a magic link to sign in or sign up instantly.
          </p>
        </div>
        ...
        <form onSubmit={handleMagicLinkSubmit} className="space-y-4">
          ...
          <button
            ...
            className="w-full py-3.5 bg-gradient-to-r from-primary-500 to-primary-600 hover:from-primary-600 hover:to-primary-700 text-white rounded-xl font-bold shadow-lg flex items-center justify-center gap-2 transition-all disabled:opacity-70 disabled:cursor-not-allowed"
          >
            {loading ? (
        ...
        <p className="mt-6 text-center text-xs text-slate-400">
          No password needed. A magic link will be sent to your inbox.
        </p>
      </div>
    </div>
  );
