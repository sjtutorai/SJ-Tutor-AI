...
const NotificationsView: React.FC<NotificationsViewProps> = ({ onNavigateToGroupInvite }) => {
  const { 
    notifications, 
    unreadCount, 
    permissionStatus, 
                      ...
                      return (
                        <motion.div
                          ...
                          onClick={() => {
                            ...
                            if (notif.metadata && notif.metadata.type === 'group_invite') {
                              if (onNavigateToGroupInvite) {
                                onNavigateToGroupInvite(notif.metadata.groupId, notif.metadata.inviterName, notif.metadata.groupName);
                              }
                            }
                          }}
                          ...
                          <div className="flex-1 min-w-0 pr-6">
                            ...
                            {notif.metadata?.type === 'group_invite' && (
                              <div className="mt-3 inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-primary-100 dark:bg-primary-900/40 text-primary-700 dark:text-primary-300 text-xs font-bold hover:bg-primary-200 dark:hover:bg-primary-900/60 transition-colors">
                                View Invitation <span aria-hidden="true">&rarr;</span>
                              </div>
                            )}
                          </div>
