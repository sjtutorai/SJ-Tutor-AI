...
const App: React.FC = () => {
  ...
  // Detect and consume Magic Link
  useEffect(() => {
    const handleMagicLink = async () => {
      if (isSignInWithEmailLink(auth, window.location.href)) {
        let email = window.localStorage.getItem('emailForSignIn');
        if (!email) {
          // Fallback if user clears cache or uses a different device/browser
        ...
        if (email) {
          try {
            ...
            setError("Failed to complete sign-in. The link might be expired or invalid.");
          } finally {
    ...
    if (dashboardView !== 'OVERVIEW') {
      ...
      return (
        <div className="relative z-10 animate-in fade-in slide-in-from-right-8 duration-500">
          <button onClick={() => setDashboardView('OVERVIEW')} className="flex items-center text-slate-500 hover:text-primary-600 mb-6 group text-sm font-medium">
      ...
      <div className="relative z-10 space-y-6">
        <div className="animate-fade-in-up bg-white/70 backdrop-blur-xl border border-white/50 rounded-2xl p-6 shadow-sm flex flex-col md:flex-row items-center gap-6 group">
          <div className="flex-1">
            ...
            <button onClick={() => { if (!user) setShowAuthModal(true); else setMode(AppMode.TUTOR); }} className="px-5 py-2.5 bg-slate-900 text-white rounded-lg font-medium flex items-center gap-2 text-sm"><MessageCircle className="w-4 h-4" /> Chat with Me</button>
          </div>
          ...
          {dashboardCards.map((stat, idx) => (
            <button key={idx} onClick={() => { if (!user) setShowAuthModal(true); else if (stat.id === AppMode.NOTES) setMode(AppMode.NOTES); else setDashboardView(stat.id as AppMode); }} className="p-4 rounded-xl bg-white border border-stone-100 shadow-sm hover:shadow-md transition-all text-left flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div><p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{stat.label}</p><span className="text-xl font-bold text-slate-800">{stat.count}</span></div>
              </div>
  ...
  const renderContent = () => {
    ...
    return (
      <div className="space-y-5 animate-in fade-in duration-500">
        ...
        {error && <div className="p-4 bg-red-50 text-red-600 rounded-lg text-sm font-medium flex items-center gap-3"><AlertCircle className="w-5 h-5" />{error}</div>}
        ...
        {!hasResult && !loading && (
          <button onClick={handleGenerate} className="w-full py-4 bg-primary-600 text-white rounded-xl font-bold shadow-lg hover:bg-primary-700 transition-all uppercase tracking-widest text-sm flex items-center justify-center gap-2"><Sparkles className="w-4 h-4" /> Generate {mode}</button>
        )}
      </div>
    );
  };
    ...
    <div className="min-h-screen bg-[#FFFAF0] flex">
      ...
      <aside className={`fixed lg:sticky top-0 left-0 z-50 h-screen w-64 bg-white border-r border-slate-200 transition-transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        <div className="h-full flex flex-col">
          <div className="p-5 border-b border-slate-100 flex items-center gap-3">
             ...
             <h1 className="text-lg font-bold text-slate-900 tracking-tight">SJ Tutor AI</h1>
          </div>
          ...
          <div className="p-3 border-t border-slate-100">
            {user ? (
               <><button onClick={() => setMode(AppMode.PROFILE)} className="w-full flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-slate-50 text-xs text-slate-600 font-medium"><div className="w-7 h-7 rounded-full bg-primary-100 flex items-center justify-center overflow-hidden">{userProfile.photoURL ? <img src={userProfile.photoURL} className="w-full h-full object-cover" /> : 'U'}</div>{userProfile.displayName || 'Scholar'}</button><button onClick={handleLogout} className="w-full flex items-center justify-center gap-2 py-2 text-xs font-medium text-red-600 mt-2 hover:bg-red-50 rounded-lg transition-colors"><LogOut className="w-3.5 h-3.5" /> Sign Out</button></>
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden">
        <header className="bg-white/80 backdrop-blur-md border-b border-slate-200 h-14 flex items-center justify-between px-5 sticky top-0 z-30">
          <div className="flex items-center gap-3"><button onClick={() => setIsSidebarOpen(true)} className="lg:hidden p-1.5 text-slate-500"><Menu className="w-5 h-5" /></button><h2 className="text-base font-bold text-slate-800">{navItems.find(n => n.id === mode)?.label || 'SJ Tutor AI'}</h2></div>
          <div className="flex items-center gap-1.5 px-2.5 py-1 bg-slate-50 border border-slate-200 rounded-full"><Zap className="w-3.5 h-3.5 text-amber-500 fill-amber-500" /><span className="text-xs font-bold text-slate-700">{userProfile.credits}</span></div>
        </header>
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 custom-scrollbar"><div className="max-w-5xl mx-auto">{renderContent()}</div></div>
      </main>
