...
import { 
  ...
  Share2,
  CreditCard,
  Loader2 // Fix: Added Loader2 to imports
} from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const calculateCost = (targetMode: AppMode, data: StudyRequestData): number => {
    ...
    if (targetMode === AppMode.ESSAY) return data.includeImages ? 15 : 10;
    if (targetMode === AppMode.QUIZ) return (data.questionCount === 20 && data.difficulty === 'Hard') ? 0 : 10;
    return 0;
  };
    ...
    try {
      if (mode === AppMode.SUMMARY) {
        ...
        if (formData.includeImages) {
          ...
          if (imageBase64) { text += `\n\n![${formData.chapterName}](${imageBase64})`; setEssayContent(text); }
        }
  ...
  /**
   * Fix: Implemented renderContent to manage navigation and view rendering based on active mode.
   */
  const renderContent = () => {
    ...
    switch (mode) {
      ...
      case AppMode.TUTOR:
        return <TutorChat onDeductCredit={deductCredit} currentCredits={userProfile.credits} context={formData} />;
