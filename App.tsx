...
const App: React.FC = () => {
  ...
  const [authLoading, setAuthLoading] = useState(() => {
    try {
      return !localStorage.getItem('sjtutor_cached_user');
    } catch {
      return true;
    }
  });
  ...
  useEffect(() => {
    if (user) {
      ...
      const loadProfile = async () => {
        // 1. Try local storage first to load INSTANTLY (0ms)
        let parsed = null;
        if (savedProfile) {
          try {
        ...
        if (parsed) {
          ...
          // Fallback to basic profile with name & email from auth object instantly
          const fallback = {
             ...initialProfileState,
        ...
        // 2. Fetch from Firestore asynchronously in the background so it doesn't block rendering
        try {
          const firestoreProfile = await getProfileFromFirestore(user.uid);
          if (firestoreProfile) {
            const merged = { ...(parsed || {}), ...firestoreProfile };
            const completeProfile = { 
              ...initialProfileState, 
              ...merged,
            ...
            // Sync fresh profile back to local storage
            localStorage.setItem(`profile_${user.uid}`, JSON.stringify(completeProfile));
            
            // Check for completion reminder (only once updated)
            ...
            // First time login - prompt reminder
          ...
          console.error("Background Firestore profile load failed:", e);
        }
      };
