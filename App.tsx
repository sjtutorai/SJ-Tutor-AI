      ...
      if (isSignInWithEmailLink(auth, window.location.href)) {
        ...
        if (email) {
          try {
            ...
            setError("Failed to sign in with magic link. It might be expired or used.");
          } finally {
      ...
      <aside className={`fixed lg:sticky top-0 left-0 z-50 h-screen w-64 bg-white border-r border-slate-200 transition-transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        <div className="h-full flex flex-col">
          ...
          <div className="p-3 border-t border-slate-100">
            {user ? (
               <><button onClick={() => setMode(AppMode.PROFILE)} className="w-full flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-slate-50 text-xs text-slate-600 font-medium">{userProfile.displayName || 'Scholar'}</button><button onClick={handleLogout} className="w-full flex items-center justify-center gap-2 py-2 text-xs font-medium text-red-600 mt-2 hover:bg-red-50 rounded-lg transition-colors">Sign Out</button></>
            ) : ( <button onClick={() => setShowAuthModal(true)} className="w-full py-2.5 bg-slate-900 text-white rounded-lg text-sm font-medium">Sign In</button> )}
          </div>
        </div>
      </aside>
